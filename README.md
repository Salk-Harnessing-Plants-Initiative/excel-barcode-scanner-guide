...
2. Open your Excel spreadsheet, and move your identifying column (barcode) to being the first column. (In our lab, this is `container_id` when we're scanning container labels, like when there's only one plant per container. Alternatively, this is `plant_id` when we're scanning plant labels, like when there's multiple plants per container and the label is directly on the plant). For this example, I'm named my main sheet `main`, and you can do the same if it makes the next steps easier.
